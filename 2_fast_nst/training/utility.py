"""
The utility functions to load the image, show the image, and do the transformation.
"""
...
def transform_net_load_image(filename, size=None, scale=None):
    """
  Load the image by the given filename.
  The image would be resized if size or scale is set.
...
def transform_net_imshow(image, title=None):
    """
  Given the PIL image from the output of the transformer net,
  transform it to the matplot format to show.
  """
    with torch.no_grad():
